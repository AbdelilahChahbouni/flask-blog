...
from flaskblog.models import User , Post, Follow
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('your Account Created Success' , 'success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password , form.password.data):
            ...
            flash("log in unsuccessfull check the email or password" , 'danger')
...
@users.route('/show_profile/<int:user_id>' , methods=['GET' , 'POST'])
@login_required
def show_profile(user_id):
    user_profile = User.query.get_or_404(user_id)
    image_file = url_for('static', filename='images/' + user_profile.profile_image)
    print(image_file)
    return render_template('show_profile.html' , user_profile=user_profile , image_file=image_file)
...
def follow(user_id):
    ...
    return redirect(url_for('users.show_profile', user_id=user.id))
